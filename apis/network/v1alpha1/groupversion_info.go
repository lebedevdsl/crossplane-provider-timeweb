...
// Group / Version metadata for the Network API group. v0.3 ships `Network`
// (a Timeweb VPC) and `FloatingIP`. Per spec.md §Clarifications (2026-06-01
// network group commitment) future network-class kinds (`Router`,
// `Balancer`, `FirewallRule`, `SecurityGroup`) extend this same group +
// same Go package — splitting into `loadbalancer.m.…` / `firewall.m.…` is
// explicitly rejected.
const (
	Group   = "network.m.timeweb.crossplane.io"
	Version = "v1alpha1"
)
...
var (
	...
	NetworkKind = "Network"
	// NetworkGroupVersionKind is the GVK for Network.
	NetworkGroupVersionKind = GroupVersion.WithKind(NetworkKind)

	// FloatingIPKind is the Kind for FloatingIP.
