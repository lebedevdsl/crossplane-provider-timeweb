...
func TestComputeSSLTable(t *testing.T) {
	...
	cases := []struct {
		...
		{"custom: no match + budget spent → exhausted (no re-upload storm)",
			sslInputs{mode: "custom", desired: desired, attempts: 4, lastAttempt: &old, now: now},
			sslActionNone, sslStateExhausted, 0},
		{"custom: no match within spacing → wait",
			sslInputs{mode: "custom", desired: desired, attempts: 1, lastAttempt: &recent, now: now},
