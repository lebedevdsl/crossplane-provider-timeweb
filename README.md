...
| `Cdn`                         | `cdn.m.timeweb.crossplane.io`          | CDN resource: one origin (`bucketRef` \| `domain` \| `ip`) behind an auto-assigned delivery domain; declared settings (cache/security/performance/CORS/headers) kept in sync; cache purge via the self-clearing `cdn.timeweb.crossplane.io/purge` annotation; custom delivery domains + SSL certificate (LE/custom), signed-URL secure token, traffic limit (see `docs/cdn.md`). |
